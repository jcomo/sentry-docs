...
Adoption stage is a high-level overview of the usage your release is seeing relative to other releases. There are three option stages: "High Adoption", "Low Adoption", and "Replaced". Every hour, we process your sessions over the last 6 hours and for each release (in a given environment and project), we'll calculate it's percentage of sessions across all sessions. If that percent is above a threshold (which is 10% divided by the number of releases in the last 6 hours), that release will be tagged as "High Adoption". If it was previously tagged as "High Adoption" but no longer meets the minimum threshold, it is labeled "Replaced". All other releases are marked as "Low Adoption". Note: "Replaced" releases that reach the threshold will once again be marked as "High Adoption".
